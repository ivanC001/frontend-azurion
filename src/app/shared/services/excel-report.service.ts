...
export class ExcelReportService {
  ...
  private addSheet(workbook: ExcelJS.Workbook, report: ExcelReportSheet): void {
    ...
    for (const sourceRow of report.rows) {
      ...
      row.eachCell((cell, colNumber) => {
        ...
        this.applyBodyCellStyle(cell, column, this.resolveCurrency(column, sourceRow));
        if (
  ...
  private applyBodyCellStyle(
    cell: ExcelJS.Cell,
    column: ExcelReportColumn,
    currency?: string,
  ): void {
    ...
    if (column.format === 'currency') {
      cell.numFmt = `"${this.currencySymbol(currency)} "#,##0.00`;
    }
  ...
  private resolveCurrency(
    ...
    const value = row[column.currencyKey];
