...
export class LandingChannelConfig implements OnInit {
  ...
  protected save(): void {
    ...
    operation.pipe(finalize(() => this.saving.set(false))).subscribe({
      next: (saved) => {
        ...
        this.message.set(
          current
            ? 'Configuración de landing actualizada.'
            : 'Landing creada. La clave ya está lista para copiar.',
        );
      },
      error: (error: unknown) => this.error.set(this.resolveError(error)),
    });
  ...
  protected regenerateKey(): void {
    ...
    this.api
      .regenerateCrmLandingKey(current.id)
      ...
      .subscribe({
        next: (saved) => {
          ...
          this.selectConfiguration(saved);
          this.message.set(
            'Landing key regenerada. Actualiza la clave en tu formulario antes de volver a enviar leads.',
          );
        },
        error: (error: unknown) => this.error.set(this.resolveError(error)),
      });
  ...
  protected regenerateRelaySecret(): void {
    ...
    this.api
      .regenerateCrmLandingRelaySecret(current.id)
  ...
  protected jsonExample(): string {
    const selectedProduct = this.publicCatalogItems().find((item) =>
      this.form.catalogoItemIds.includes(item.id),
    );
