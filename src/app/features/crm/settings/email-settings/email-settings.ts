...
export class EmailSettings implements OnInit {
  ...
  protected sendTest(): void {
    ...
    this.service
      ...
      .subscribe({
        ...
        error: (error: unknown) =>
          this.error.set(this.resolveError(error) || 'No se pudo enviar el correo de prueba.'),
      });
  }
  ...
  protected toggleActive(): void {
    ...
    operation.pipe(finalize(() => this.toggling.set(false))).subscribe({
      next: (saved) => {
        ...
        this.message.set(
          saved.activo
            ? 'Configuracion de correo activada.'
            : 'Configuracion de correo desactivada.',
        );
      },
      error: (error: unknown) => this.error.set(this.resolveError(error)),
    });
  ...
  private isValidForm(): boolean {
    if (
      ...
      !this.form.correoRemitente.trim() ||
