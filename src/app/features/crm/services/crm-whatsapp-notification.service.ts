...
import { WhatsappUnreadSummary } from '@features/crm/data/crm-api.types';
...
export class CrmWhatsappNotificationService {
    ...
    this.api
      ...
      .subscribe({
        next: (summary) => {
          ...
          const hasNewMessage =
            Number(summary.mensajesNoLeidos || 0) > Number(previous.mensajesNoLeidos || 0) ||
          ...
          if (notify && hasNewMessage && summary.mensajesNoLeidos > 0) {
            ...
            const detail =
              summary.ultimoMensaje?.trim() || 'Tienes un mensaje nuevo en la bandeja de WhatsApp.';
  ...
  private canReadWhatsapp(): boolean {
    return (
      this.session.hasModule('CRM') &&
      (this.session.hasPermission('CRM_LEADS_READ') ||
        this.session.hasPermission('CRM_ACTIVITIES_READ'))
    );
  }
}
