...
import type { CrmActividad, CrmOportunidad, CrmProspecto } from '@features/crm/data/crm-api.types';
...
export class CrmLiveUpdateService {
  ...
  watch(
    refreshIntervalMs = CRM_LIVE_REFRESH_MS,
    shouldRefresh: () => boolean = () => true,
  ): Observable<CrmLiveSnapshot> {
    return timer(refreshIntervalMs, refreshIntervalMs).pipe(
      filter(() => !this.document.hidden && shouldRefresh()),
      exhaustMap(() =>
        forkJoin({
          prospectos: this.prospects
            .page({ page: 0, size: CRM_LIVE_PAGE_SIZE })
            .pipe(map((page) => page.content)),
          oportunidades: this.opportunities
            .page({ page: 0, size: CRM_LIVE_PAGE_SIZE })
            .pipe(map((page) => page.content)),
          actividades: this.followups
            .pageActivities({ page: 0, size: CRM_LIVE_PAGE_SIZE })
            .pipe(map((page) => page.content)),
