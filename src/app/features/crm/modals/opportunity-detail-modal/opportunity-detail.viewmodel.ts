...
export function buildOpportunityRequirementChecklist(
  ...
  const hasSentOrDeliveredQuote = snapshot.quotes.some(
    (quote) => SENT_QUOTE_STATES.has(quoteStatusValue(quote)) || Boolean(quote.canalEnvio),
  );

  return [
    { label: 'Cliente definido', done: hasContact },
    { label: 'Requerimiento / Item definido', done: hasNamedOffer },
    { label: 'Cantidad definida', done: hasQuantity },
    { label: 'Valor estimado', done: hasValue },
    { label: 'Cotización creada', done: snapshot.quotes.length > 0 },
    { label: 'Cotización enviada o descargada', done: hasSentOrDeliveredQuote },
  ];
}
