...
Este modulo esta entrando en una migracion gradual para que `crm-page` quede como contenedor del CRM.
...
- `crm-page.ts` conserva las mismas claves de almacenamiento y delega lectura/escritura al servicio, sin cambiar rutas ni endpoints.
- La compilacion `npm run build` pasa despues de estos cambios.

Advertencias pendientes de la compilacion actual:

- `crm-page.scss` excede el presupuesto configurado por aproximadamente 30 kB.
