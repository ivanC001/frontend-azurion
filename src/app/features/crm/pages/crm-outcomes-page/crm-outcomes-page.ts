...
>>>>>>> b50118bff6d4d47a3981a187eab708420ee804bc
  ...
  protected readonly resultOptions: ReadonlyArray<{
    label: string;
    value: OutcomeFilter;
    icon: string;
  }> = [
  ...
  protected readonly metrics = computed(() => {
    ...
    return [
      {
        label: 'Cierres evaluados',
        value: String(closed),
        detail: 'ganadas y perdidas',
        icon: 'pi pi-flag',
        tone: 'blue',
      },
      {
  ...
  private resolveError(error: unknown): string {
    if (typeof error === 'object' && error !== null && 'error' in error) {
      ...
      return (
        payload?.details?.[0] ||
        payload?.message ||
        'No se pudieron cargar los resultados comerciales.'
      );
    }
