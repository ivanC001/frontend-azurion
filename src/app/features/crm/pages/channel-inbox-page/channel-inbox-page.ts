...
const CHANNEL_VIEWS: Record<InboxChannelCode, InboxChannelView> = {
  FACEBOOK: {
    ...
    detail:
      'El canal queda disponible cuando las credenciales y el webhook firmado han sido configurados.',
  ...
  CORREO: {
    ...
    detail:
      'Por ahora esta bandeja muestra exclusivamente mensajes enviados. La recepcion se habilitara cuando exista un webhook de correo.',
...
export class ChannelInboxPage implements OnInit {
  ...
  protected readonly view = computed(
    () => CHANNEL_VIEWS[this.channelCode()] ?? CHANNEL_VIEWS.FACEBOOK,
  );
  ...
  private loadSentEmails(): void {
    ...
    this.emailError.set('');
    this.api
      .pageCrmSentEmails(this.emailQuery(), this.emailPage(), 20)
